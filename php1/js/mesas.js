  ...
  formas_pago();
    ...
    $("#txt_ci" ).autocomplete({
      source: function( request, response ) {
            ...
            $.ajax({
                url: '../controlador/mesasC.php?buscar_ci=true',      
                type: 'get',
                dataType: "json",
                data: {
                    q1: request.term
        ...
        select: function (event, ui) {
            ...
            $('#txt_ci').val(ui.item.ci); // save selected id to input
            $('#txt_email').val(ui.item.mail); // save selected id to input
            $('#txt_direccion').val(ui.item.dir); // save selected id to input
            $('#txt_telefono').val(ui.item.tel); // save selected id to input
            $('#txt_razon').val(ui.item.nombre); // save selected id to input
            $('#txt_id_cliente').val(ui.item.value); // save selected id to input
            return false;
        },
...
function generar_mesas()
{
    ...
    url:  '../controlador/mesasC.php?generar_mesas=true',      
    type:  'post',
    dataType: 'json',
...
function modal_cantidad(articulo)
{
	$('#cantidad_modal').modal('show');
	$('#txt_id_art').val(articulo);
}
function add_articulo()
{
	if($('#txt_can').val()=='' || $('#txt_can').val()==0)
	{
	...
	var parametros = 
	{
		'articulo':$('#txt_id_art').val(),
		'cantidad':$('#txt_can').val(),
		'mesa':id,
    'llevar':$('input[name="rbl_llevar"]:checked').val(),
	}
	 $.ajax({
    data:  {parametros:parametros},
    url:  '../controlador/mesasC.php?add_mesa=true',      
    type:  'post',
    dataType: 'json',
    success:  function (response) { 
     if(response==1)
...
function cargar_mesa(id)
{
	var parametros = 
	{
	 ...
	 $.ajax({
    data:  {parametros:parametros},
    url:  '../controlador/mesasC.php?cargar_mesa=true',      
    type:  'post',
    dataType: 'json',
    success:  function (response) { 
